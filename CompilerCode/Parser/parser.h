...
public:
	...
	void idref(bool left)
	{
		...
		if (identifiers->find(tokens.front().tokenName)->name != "Not Found")
		{
			...
			if (!left)
			{
				...
				assem << "lw $t" << currTemp << " " << identifiers->find(tokens.front().tokenName)->address << " # load value\n";
	...
	void factorprime()
	{
		...
		if (tokens.front().tokenNumber == 6)
		{
			...
			assem << "lw $t" << currTemp << " " << currentAddr << endl;
			currentAddr += 4;
			currTemp++;
			assem << "lw $t" << currTemp << " " << currentAddr << endl;

			if (temp == "<")
				assem << "blt $t" << currTemp << " $t" << currTemp - 1 << " Skip" << endl;

			else if (temp == ">")
				assem << "bgt $t" << currTemp << " $t" << currTemp - 1 << " Skip" << endl;

			else if (temp == "!=")
				assem << "bne $t" << currTemp << " $t" << currTemp - 1 << " Skip" << endl;
	...
	void assstat()
	{
		...
		assem << "lw $t" << currTemp << " " << currentAddr << endl;
		assem << "sw $t" << currTemp << " " << loc << " # assign to variable" << endl;
		currTemp = 0;
	}
